...
class SRTM(Command):
    ...
    option_list = (
        Option(
            "--tile", nargs=2, type=int, metavar=("x", "y"), help="Import SRTM tile x y"
        ),
        Option(
            "--file",
            nargs="+",
            metavar=("filename"),
            help="Import tile from local file(s)",
        ),
        Option(
            "--in-db", action="store_true", help="Store data in-db instead of out-of-db"
        ),
    )
    ...
    def raster2pgsql(self, tiled_tif_filename, in_db):
        ...
        args = [
            ...
            "-t",
            "100x100",  # 100x100 tiles
        ]

        if not in_db:
            args.append("-R")  # Out-of-DB raster

        args += [os.path.abspath(tiled_tif_filename), "elevations"]
