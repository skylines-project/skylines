...
def test_get_live_filter_by_from_time(db_session, client):
    """The default max_age is 12 hours"""
    utcnow = datetime(year=2020, month=12, day=20, hour=20)
    from_time = utcnow - timedelta(hours=1)
    ...
    with patch("skylines.model.tracking.datetime") as datetime_mock:
        ...
        res = client.get(
            "/live/{john},{jane}?from_time={from_time}".format(
                john=john.id, jane=jane.id, from_time=to_timestamp(from_time)
            )
        )
        ...
        assert len(json[u"flights"]) == 1

        expected_fixes = list(filter(lambda f: f.time >= from_time, john_fixes))
        assert decode_time(
            json[u"flights"][0][u"barogram_t"]
        ) == get_fixes_times_seconds(expected_fixes)

        # Can not get fixes aged more than 12h
        res = client.get(
            "/live/{john},{jane}?from_time=0".format(john=john.id, jane=jane.id)
        )
