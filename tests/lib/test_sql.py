...
def test_query_to_sql(db_session):
    ...
    query = db_session.query(ExampleTable) \
    ...
    assert sql_text == "SELECT ilike_test.id, ilike_test.name, ilike_test.uni \n" + \
        "FROM ilike_test \n" + \
...
def test_query_to_sql2(db_session):
    ...
    assert is_unicode(sql_text)
    assert sql_text == u"SELECT ST_GeomFromWKB('\\001\\002\\000\\000\\000\\003\\000\\000\\000\\000\\000\\000\\000" + \
